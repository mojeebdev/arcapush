    ...
    <div className="space-y-8">
      <div className="flex items-center gap-2 overflow-x-auto pb-4 no-scrollbar">
        ...
        <button 
          ...
          className="ml-auto p-3 bg-zinc-900 border border-white/5 hover:bg-zinc-800 rounded-full transition-all"
        >
      ...
      {loading ? (
        ...
        <div className="bg-zinc-950 border border-white/5 rounded-[3rem] p-32 text-center shadow-2xl">
          ...
          <p className="text-zinc-700 font-black uppercase tracking-[0.5em] text-[10px]">
            The Stream is Quiet
          </p>
        </div>
      ) : (
        <div className="grid gap-6">
          {requests.map((req) => (
            <div 
              key={req.id} 
              className="group bg-zinc-950 border border-white/5 rounded-[2.5rem] p-8 hover:border-[#4E24CF]/30 transition-all shadow-xl"
            >
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8">
                <div className="space-y-3">
                  <div className="flex items-center gap-4">
                    ...
                    <span className="text-[9px] font-black text-[#4E24CF] uppercase tracking-[0.2em]">
                      {req.requesterRole} Access
                    </span>
                  </div>
                ...
                {req.status === "PENDING" ? (
                  <div className="flex items-center gap-4">
                    <button
                      ...
                      className="px-12 py-4 bg-white text-black rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] hover:bg-[#D4AF37] transition-all shadow-2xl active:scale-95"
                    >
                      Authorize
                    </button>
                    <button
                      onClick={() => handleAction(req.id, "REJECTED")}
                      className="p-4 bg-zinc-900 text-zinc-600 hover:text-red-500 border border-white/5 rounded-2xl transition-all"
                    >
  ...
  if (!isAuthorized) {
    return (
      <div className="min-h-screen bg-black flex flex-col items-center justify-center text-white p-6 relative">
        <Toaster position="top-center" toastOptions={{ style: { background: '#09090b', color: '#fff', border: '1px solid #27272a' } }} />
        ...
        <header className="flex flex-col md:flex-row md:items-end justify-between border-b border-white/5 pb-16 gap-8">
          <div className="space-y-4">
            ...
            <h1 className="text-5xl md:text-7xl font-black tracking-tighter uppercase italic leading-[0.8]">
              Whate <span className="text-[#4E24CF]">Engine</span>
            </h1>
          </div>
          <button 
            onClick={() => setIsAuthorized(false)} 
            className="text-[9px] font-black tracking-[0.3em] uppercase px-10 py-4 border border-white/5 rounded-full bg-zinc-950 hover:bg-red-500/10 hover:text-red-500 hover:border-red-500/20 transition-all shadow-2xl"
          >
            Lock Terminal
          </button>
        </header>
