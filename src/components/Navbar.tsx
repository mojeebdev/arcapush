...
export function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 border-b border-black/8 bg-[#F5F0E8]/90 backdrop-blur-md">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="flex items-center justify-between h-20 gap-8">
          ...
          <div className="hidden lg:flex items-center gap-6">
            {NAV_LINKS.map((link) => (
              <Link
                ...
                className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-500 hover:text-zinc-900 transition-colors"
              >
                {link.label}
              </Link>
            ))}
            <HiOutlineShieldCheck className="w-4 h-4 text-zinc-300 hover:text-[#4E24CF] transition-colors" />
          </div>
          ...
          <div className="hidden md:flex items-center gap-4 shrink-0">
            {mounted && (
              <div className="flex items-center gap-3 pr-4 border-r border-black/10">
                ...
                <button
                  ...
                  className="flex items-center gap-2 text-[9px] font-black uppercase tracking-[0.3em] text-zinc-500 hover:text-zinc-900 transition-colors group"
                >
            ...
            <Link
              href="/submit"
              className="bg-zinc-900 text-white rounded-xl text-[10px] font-black px-6 py-2.5 uppercase tracking-widest hover:bg-[#4E24CF] hover:scale-[1.05] transition-all shadow-sm active:scale-95"
            >
              Get Started
            </Link>
          </div>
          ...
          {/* Mobile Toggle */}
          <button onClick={() => setMobileOpen(!mobileOpen)} className="md:hidden p-2 rounded-lg text-zinc-600">
      ...
      {/* Mobile Menu */}
      {mobileOpen && (
        <div className="md:hidden border-t border-black/8 bg-[#F5F0E8]/98 backdrop-blur-xl px-6 py-6 flex flex-col gap-5">
          {NAV_LINKS.map((link) => (
            <Link
              key={link.href} href={link.href}
              onClick={() => setMobileOpen(false)}
              className="text-[11px] font-black uppercase tracking-[0.3em] text-zinc-600 hover:text-zinc-900 transition-colors"
            >
              {link.label}
            </Link>
          ))}
