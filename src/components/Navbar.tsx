...
export function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 border-b border-white/5 bg-black/80 backdrop-blur-md">
      ...
      {/* Mobile Menu */}
      {mobileOpen && (
        <div className="md:hidden border-t border-white/5 bg-black/95 backdrop-blur-xl px-6 py-6 flex flex-col gap-5">
          {NAV_LINKS.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              onClick={() => setMobileOpen(false)}
              className="text-[11px] font-black uppercase tracking-[0.3em] text-zinc-400 hover:text-white transition-colors"
            >
              {link.label}
            </Link>
          ))}
          ...
          <Link
            href="/submit"
            onClick={() => setMobileOpen(false)}
            className="w-full bg-white text-black rounded-xl text-[10px] font-black px-6 py-3 uppercase tracking-widest hover:bg-[#D4AF37] transition-all text-center"
          >
            Get Started
          </Link>
        </div>
