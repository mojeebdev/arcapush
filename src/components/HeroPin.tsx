...
import {
  HiOutlineFire, HiOutlineArrowRight, HiOutlineBolt,
  HiOutlineChevronLeft, HiOutlineChevronRight,
} from "react-icons/hi2";

interface HeroPinProps { startups: any[]; }
  ...
  if (startups.length === 0) {
    return (
      <div className="relative rounded-[2.5rem] overflow-hidden border border-black/8 bg-white shadow-card min-h-[420px] flex flex-col items-center justify-center">
        ...
        {showPayment && (
          <PaymentModal onClose={() => setShowPayment(false)} startupId="hero-empty-slot" status="APPROVED" onSuccess={() => window.location.reload()} />
        )}
      </div>
    );
  }
        ...
        >
          ...
          <div className="relative h-[400px] sm:h-[480px] lg:h-[600px]">
            ...
            <div className="absolute top-8 left-8 z-20 flex gap-3">
              <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-[#D4AF37] text-black shadow-sm">
            ...
            <div className="absolute bottom-0 left-0 right-0 p-8 sm:p-12 z-20">
              <div className="max-w-3xl">
                ...
                <div className="flex flex-wrap items-center gap-4">
                  <Link href={`/startup/${currentItem.id}`}
                    className="px-10 py-5 bg-zinc-900 text-white rounded-2xl font-black text-xs uppercase tracking-[0.2em] hover:bg-[#4E24CF] transition-all duration-300 shadow-sm">
                    Request Access
                    <HiOutlineArrowRight className="w-4 h-4 inline ml-2" />
                  </Link>
                  <ShareButton startup={currentItem} />
                  {currentItem.website && (
                    <a href={currentItem.website} target="_blank" rel="noopener noreferrer"
                      className="px-10 py-5 bg-white/70 border border-black/10 text-zinc-700 rounded-2xl font-black text-xs uppercase tracking-[0.2em] hover:border-[#D4AF37]/50 hover:text-zinc-900 transition-all backdrop-blur-md">
                      Visit Signal
                    </a>
                  )}
                </div>
          ...
          {total > 1 && (
            <>
              <button onClick={prev} className="absolute left-6 top-1/2 -translate-y-1/2 z-30 w-12 h-12 rounded-full bg-white/80 backdrop-blur-xl border border-black/10 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all hover:bg-[#4E24CF] hover:text-white text-zinc-700 shadow-sm">
                <HiOutlineChevronLeft className="w-5 h-5" />
              </button>
              <button onClick={next} className="absolute right-6 top-1/2 -translate-y-1/2 z-30 w-12 h-12 rounded-full bg-white/80 backdrop-blur-xl border border-black/10 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all hover:bg-[#4E24CF] hover:text-white text-zinc-700 shadow-sm">
