...
export default function AboutPage() {
  return (
    <main className="pt-32 pb-24 px-6 max-w-5xl mx-auto" style={{ backgroundColor: "#F5F0E8" }}>

      {/* Hero Section */}
      <section className="mb-24">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
          <span className="text-[#D4AF37] text-[10px] font-black uppercase tracking-[0.4em]">The Origin Story</span>
          <h1 className="text-5xl md:text-7xl font-black text-zinc-900 uppercase tracking-tighter leading-[0.9]">
            From a Spark on X to the <span className="text-[#4E24CF]">Limelight.</span>
          </h1>
          <p className="text-xl text-zinc-600 font-medium leading-relaxed max-w-3xl">
            It started with a realization: the art of marketing is becoming as essential as the art of code.
            Vibestream was built to ensure no <span className="text-zinc-900 italic">"Vibe Code"</span> remains in the shadows.
          </p>
        </motion.div>
      </section>

      {/* Mission & Vision */}
      <section className="grid grid-cols-1 md:grid-cols-2 gap-px bg-black/8 border border-black/8 rounded-[3rem] overflow-hidden mb-32 shadow-card-lg">
        <div className="bg-white p-12 flex flex-col justify-between group">
          <div className="space-y-6">
            ...
            <h2 className="text-xs font-black text-zinc-400 uppercase tracking-[0.3em]">Our Mission</h2>
            <p className="text-3xl font-black text-zinc-900 uppercase tracking-tighter">
              One Startup <br />
        ...
        <div className="bg-white p-12 flex flex-col justify-between group">
          <div className="space-y-6">
            ...
            <h2 className="text-xs font-black text-zinc-400 uppercase tracking-[0.3em]">Our Vision</h2>
            <p className="text-3xl font-black text-zinc-900 uppercase tracking-tighter">
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-32">
        {[
          ...
          { icon: <HiOutlineMagnifyingGlass className="w-8 h-8 text-zinc-600 group-hover:scale-110 transition-transform" />, title: "Encyclopedia", border: "hover:border-zinc-300", text: "The definitive directory of Vibe Code. Search for any problem, find the perfect productivity solution." },
        ].map((pillar) => (
          <div key={pillar.title} className={`p-8 rounded-[2rem] bg-white border border-black/8 space-y-4 ${pillar.border} transition-all group shadow-card`}>
            {pillar.icon}
            <h3 className="text-lg font-black text-zinc-900 uppercase tracking-tighter">{pillar.title}</h3>
            <p className="text-xs text-zinc-500 font-medium leading-relaxed uppercase">{pillar.text}</p>
          </div>
        ))}
      </section>
    </main>
