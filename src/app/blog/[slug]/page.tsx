...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: { title: post.title, description: post.description, url, siteName: AdminConfig.SITE_NAME, type: "article", publishedTime: post.date, authors: [post.author], images: [{ url: post.image, width: 1200, height: 630 }] },
    twitter: { card: "summary_large_image", site: AdminConfig.BRAND_TWITTER, creator: AdminConfig.FOUNDER_TWITTER, title: post.title, description: post.description, images: [post.image] },
  };
}
...
export default async function BlogPostPage({ params }: Props) {
  ...
  const jsonLd = {
    ...
    headline: post.title, description: post.description, datePublished: post.date, image: post.image,
    author: { "@type": "Person", name: post.author, url: post.authorUrl, sameAs: [AdminConfig.FOUNDER_TWITTER, post.authorUrl] },
    publisher: { "@type": "Organization", name: AdminConfig.SITE_NAME, url: AdminConfig.SITE_URL, logo: `${AdminConfig.SITE_URL}/logo.png` },
    mainEntityOfPage: { "@type": "WebPage", "@id": `${AdminConfig.SITE_URL}/blog/${post.slug}` },
      ...
      <main className="flex-grow">
        ...
        <header className="pt-8 pb-12 px-6 max-w-3xl mx-auto border-b border-white/8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
            <div className="flex items-center gap-4">
              {post.authorImage ? (
                <Image src={post.authorImage} alt={post.author} width={36} height={36}
        ...
        <article className="px-6 max-w-3xl mx-auto py-14">
          <ReactMarkdown
            components={{
              h2: ({ children }) => (
                <h2 className="text-2xl md:text-3xl font-black uppercase tracking-tighter text-zinc-900 mt-16 mb-6 border-b border-black/8 pb-4">{children}</h2>
              ),
              h3: ({ children }) => (
                <h3 className="text-xl font-black uppercase tracking-tighter text-zinc-900 mt-10 mb-4">{children}</h3>
              ),
              p: ({ children }) => (
                <p className="text-zinc-600 text-base leading-[1.9] mb-6">{children}</p>
              ),
              strong: ({ children }) => (
                <strong className="text-zinc-900 font-black">{children}</strong>
              ),
              a: ({ href, children }) => (
                <a href={href} target="_blank" rel="noopener noreferrer"
                  className="text-[#4E24CF] hover:text-[#D4AF37] transition-colors underline underline-offset-4">{children}</a>
              ),
              ...
              li: ({ children }) => (
                <li className="text-zinc-600 text-base leading-relaxed flex gap-3 items-start">
                  <span className="text-[#D4AF37] mt-1.5 text-[8px] flex-shrink-0">◆</span>
                  <span>{children}</span>
                </li>
              ),
              blockquote: ({ children }) => (
                <blockquote className="border-l-2 border-[#4E24CF] pl-6 my-8 italic text-zinc-700 text-lg leading-relaxed">{children}</blockquote>
              ),
              // Keep code blocks dark — they look better dark on off-white
              code: ({ children }) => (
                <code className="bg-zinc-900 text-[#D4AF37] rounded px-2 py-0.5 text-sm font-mono">{children}</code>
              ),
              pre: ({ children }) => (
                <pre className="bg-zinc-900 border border-zinc-700 rounded-2xl p-6 overflow-x-auto my-8 text-sm">{children}</pre>
              ),
              table: ({ children }) => (
