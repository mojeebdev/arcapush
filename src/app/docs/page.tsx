...
export default function DocsPage() {
  return (
    <div className="max-w-4xl mx-auto py-20 px-6" style={{ backgroundColor: "#0f0f12" }}>
      <div className="mb-20 text-center">
        <h1 className="text-6xl font-black uppercase italic tracking-tighter mb-6 text-white">
      ...
      <div className="grid gap-8">
        {sections.map((section, index) => (
          <div key={index} className="group relative p-8 rounded-[2rem] border border-white/8 bg-[#16161b] transition-all duration-500 shadow-card hover:shadow-card-lg">
            <div className="flex items-start gap-6">
              <div className="p-4 rounded-2xl bg-[#1a1a21] border border-white/8 group-hover:border-[#D4AF37]/40 transition-colors">
                {section.icon}
              </div>
              <div className="flex-grow">
                <div className="flex items-center justify-between mb-2">
                  <h2 className="text-xl font-black uppercase tracking-widest text-white">{section.title}</h2>
                </div>
                <p className="text-gray-400 leading-relaxed font-medium mb-8 text-sm max-w-2xl">{section.content}</p>
                <Link href={section.href}
                  ...
                  <span className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-300 group-hover/link:text-[#D4AF37] transition-colors">
      ...
      {/* Safety Protocol */}
      <div className="mt-20 p-10 rounded-[3rem] bg-[#16161b] border border-white/8 relative overflow-hidden group shadow-card">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left">
          <div className="p-5 rounded-xl bg-[#1a1a21] border border-white/8">
            <h4 className="text-[10px] font-black text-white uppercase mb-2 tracking-widest">Non-Custodial</h4>
            <p className="text-gray-400 text-[10px] leading-relaxed">VibeStream never stores private keys. All transactions are signed locally via your browser extension (Phantom/MetaMask).</p>
          </div>
          <div className="p-5 rounded-xl bg-[#1a1a21] border border-white/8">
            <h4 className="text-[10px] font-black text-white uppercase mb-2 tracking-widest">Encrypted Transmission</h4>
            <p className="text-gray-400 text-[10px] leading-relaxed">Sensitive metadata is encrypted via industry-standard SSL and gated by institutional verification filters.</p>
          </div>
        </div>
      </div>

      <div className="mt-16 text-center">
        <p className="text-gray-400 mb-8 font-playfair italic">Our Guardians are standing by for institutional inquiries.</p>
        <a href="mailto:blindspotlabs1@gmail.com"
          className="bg-[#4E24CF] hover:bg-[#6B3FE0] text-white px-10 py-4 rounded-xl font-black uppercase text-[10px] tracking-widest transition-all duration-500">
          Contact Support
        </a>
      </div>
    </div>
